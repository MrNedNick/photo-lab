...
export interface Project { file: Blob; name: string; entries: Edit[]; index: number; updated: number }
...
export async function readProject(): Promise<Project | undefined> {
  ...
  try { return await new Promise((resolve, reject) => {
    const tx = db.transaction('projects', 'readonly'), request = tx.objectStore('projects').get('current')
    request.onsuccess = () => resolve(request.result)
    request.onerror = () => reject(request.error)
  }) } finally { db.close() }
}
