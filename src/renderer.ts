...
export class Renderer {
  ...
  constructor(canvas: HTMLCanvasElement | OffscreenCanvas) {
    this.canvas = canvas
    const gl = canvas.getContext('webgl2', { preserveDrawingBuffer: true, premultipliedAlpha: false }) as WebGL2RenderingContext | null
    if (!gl) throw new Error('WebGL2 is unavailable. Enable hardware acceleration or try a current Chrome, Firefox or Safari browser.')
    this.gl = gl
  ...
  load(bitmap: ImageBitmap) {
    const gl = this.gl
    if (Math.max(bitmap.width, bitmap.height) > gl.getParameter(gl.MAX_TEXTURE_SIZE)) throw new Error('This photo exceeds your GPU texture limit. Try a smaller image.')
    this.width = bitmap.width; this.height = bitmap.height
    gl.bindTexture(gl.TEXTURE_2D, this.texture)
    gl.texImage2D(gl.TEXTURE_2D, 0, gl.RGBA, gl.RGBA, gl.UNSIGNED_BYTE, bitmap)
    if (gl.getError() !== gl.NO_ERROR) throw new Error('Your GPU could not load this photo. Try a smaller image.')
  }
  render(edit: Edit, max = 2048) {
    const gl = this.gl, size = dimensions(this.width, this.height, edit, max)
    ...
    gl.uniform4f(loc('crop'), edit.crop.x, edit.crop.y, edit.crop.width, edit.crop.height)
    gl.uniform1i(loc('rotation'), ((edit.rotation / 90) % 4 + 4) % 4)
    gl.uniform2f(loc('flip'), +edit.flipX, +edit.flipY)
    gl.uniform4f(loc('adjustment'), edit.exposure, edit.contrast, edit.saturation, edit.temperature)
